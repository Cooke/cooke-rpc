...
namespace CookeRpc.AspNetCore
{
    ...
    public class RpcIntrospectionHttpMiddleware
    {
        ...
        private readonly JsonSerializerOptions _introspectionSerializerOptions =
            new()
            {
        ...
        public RpcIntrospectionHttpMiddleware(
            RequestDelegate next,
            RpcIntrospectionHttpMiddlewareOptions options
        )
        {
            _next = next;
            _options = options;
        ...
        private async Task ProcessIntrospectionRequest(HttpContext context)
        {
            ...
            static object GetTypeUsage(IRpcType type) =>
                type switch
                {
                    ...
                    UnionRpcType unionType
                        => new { kind = "union", types = unionType.Types.Select(GetTypeUsage) },
                    GenericRpcType genericType
                        => new
                        {
                            kind = "generic",
            ...
            static object GetTypeDeclaration(INamedRpcType type) =>
                type switch
                {
                    PrimitiveRpcType primType => new { primType.Name, kind = "primitive", },
                    EnumRpcType e
                        => new
                        {
                            name = e.Name,
                    ...
                    ObjectRpcType obj
                        => new
                        {
                            name = obj.Name,
                            kind = "object",
                            properties = obj.Properties.Count == 0
                                ? null
                                : obj.Properties.Select(GetIntrospectionProperty),
                            extends = obj.Extends.Count == 0
                                ? null
                                : obj.Extends.Select(GetTypeUsage),
                            @abstract = obj.IsAbstract,
                            typeParameters = obj.TypeParameters.Count == 0
                                ? null
                                : obj.TypeParameters.Select(GetTypeUsage)
                        },
                    NamedUnionRpcType ass
                        => new
                        {
                            name = ass.Name,
                            kind = "union",
